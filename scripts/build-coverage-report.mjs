...
const cols = Object.fromEntries(registry.columns.map((name, i) => [name, i]));
...
const rows = registry.metrics.map((row) => {
  ...
  return { ...item, coverageStatus, observedStatus: observed?.status ?? null, sourceDate: observed?.sourceDate ?? null };
});
